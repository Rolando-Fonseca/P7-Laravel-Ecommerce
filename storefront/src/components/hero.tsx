...
/**
 * El bloque visual no es una foto de banco: son los `color_hex` reales de las
 * variantes del catálogo, apilados. Cada banda corresponde a un SKU que existe
 * en la base de datos.
 *
 * Es una decisión, no una carencia. Una foto de stock de un modelo con chaqueta
 * no dice nada del proyecto; una franja de color etiquetada con su SKU sí.
 */
const swatches = products
  .filter((p) => p.sample_variant?.color.hex)
  .slice(0, 5)
  .map((p) => ({
    hex: p.sample_variant!.color.hex as string,
  ...
  return (
    <section id="top" className="relative overflow-hidden bg-surface">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 px-5 pb-section pt-16 sm:px-8 lg:grid-cols-12 lg:gap-16 lg:pt-24">
        ...
        {/* Columna visual: 5 de 12, y se sale del margen inferior a propósito. */}
        <Reveal delay={0.1} className="lg:col-span-5">
          <div className="relative h-full min-h-[26rem] overflow-hidden rounded-sm border border-line bg-background lg:min-h-[34rem]">
            <div className="flex h-full flex-col">
              {swatches.map((s, i) => {
                const tono = inkOn(s.hex);

                return (
                  <div
                    key={s.sku}
                    className="relative flex flex-1 items-end justify-between gap-4 px-6 py-4"
                    style={{ backgroundColor: s.hex }}
                  >
                    <span className={`text-fluid-xs font-medium ${tono.strong}`}>
                      {s.name}
                    </span>
                    <span
                      className={`tabular text-[10px] uppercase tracking-[0.14em] ${tono.soft}`}
                    >
                      {s.sku}
                    </span>
                    {i === 0 && (
                      <span
                        className={`absolute left-6 top-4 text-fluid-xs uppercase tracking-[0.24em] ${tono.soft}`}
                      >
                        color_hex
                      </span>
                    )}
